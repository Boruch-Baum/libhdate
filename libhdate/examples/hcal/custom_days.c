...
//	int		end_of_input_file = FALSE;
	...
	int		leap_adj = 0;
	int 	key_cheshvan_30 = -1;
	int 	key_kislev_30   = -1;
	int 	key_february_29 =  1;
	int 	key_adar_II     = -1;
	int 	key_adar_II_in_leap_year = 2;
	...
	* sub-procedure for read_custom_days_file()
	************************************************************/
	void parse_option_line()
	{
		char* input_key;
		int   input_value;
		const int	num_of_keys = 5;
		const char*	key_list[] = {
			"CHESHVAN_30",
			"KISLEV_30",
			"FEBRUARY_29",
		...
		if (match_count == 2)
		{
			for (i=0; i<num_of_keys; i++)
			{
				if (strcmp(input_key, key_list[i]) == 0)
				{
					switch(i)
					{
/** CHESHVAN_30 */	case  0: if ((input_value >= -1) && (input_value <= 1)) key_cheshvan_30 = input_value;
							 break;
/** KISLEV_30   */	case  1: if ((input_value >= -1) && (input_value <= 1)) key_kislev_30 = input_value;
							 break;
/** FEBRUARY_29 */	case  2: if ((input_value >= -1) && (input_value <= 1)) key_february_29 = input_value;
							 break;
/** ADAR_II     */	case  3: if ((input_value >= -1) && (input_value <= 1)) key_adar_II = input_value;
							 break;
/** ADAR_II_IN_LEAP_YEAR */
					case  4: if ((input_value == 1) || (input_value == 2)) key_adar_II_in_leap_year = input_value;
							 break;
					} /// end switch
			...
			************************************************************/
			...
			case 'G':
					...
					(!validate_hdate(CHECK_DAY_PARM, custom_day_of_month, custom_month, h1.gd_year, TRUE, &h1))	)
				{
					...
					else if (key_february_29 == -1) leap_adj = -1;
				}
				else
				{
