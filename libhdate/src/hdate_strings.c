...
char* hdate_string( int const type_of_string, int const index, int const input_short_form, int const input_hebrew_form)
{
	...
	static char *parashaot[2][2][62] = {
		...
		{ /// begin hebrew long
		 ...
		 "מטות",		"מסעי",			"דברים",
		 "ואתחנן",		"עקב",			"ראה",
		 "שופטים",		"כי_תצא",		"כי_תבוא",		/* 50 */
		 "נצבים",		"וילך",			"האזינו",
		 "וזאת_הברכה",	/* 54 */
		 "ויקהל-פקודי",	"תזריע-מצורע",	"אחרי_מות-קדושים",
...
/**  0 **/ N_("regular_day"),
		...
		{ /// begin hebrew long
		 "יום_חול",
		 "א'_ראש_השנה",		"ב'_ראש_השנה",
		 "צום_גדליה",		"יום_הכפורים",
		 "סוכות",		"חול_המועד_סוכות",
		 "הושענא_רבה",		"שמחת_תורה",
		 "חנוכה",		"צום_עשרה_בטבת",/* 10 */
		 "ט\"ו_בשבט",		"תענית_אסתר",
		 "פורים",		"שושן_פורים",
		 "פסח",			"חול_המועד_פסח",
		 "יום_העצמאות",		"ל\"ג_בעומר",
		 "ערב_שבועות",		"שבועות",	/* 20 */
		 "צום_שבעה_עשר_בתמוז",	"תשעה_באב",
		 "ט\"ו_באב",		"יום_השואה",
		 "יום_הזכרון",		"יום_ירושלים",
		 "שמיני_עצרת",		"שביעי_פסח",
		 "אחרון_של_פסח",	"שני_של_שבועות",/* 30 */
		 "שני_של_סוכות",	"שני_של_פסח",
		 "יום_המשפחה",		"יום_זכרון...", 
		 "יום_הזכרון_ליצחק_רבין","יום_ז\'בוטינסקי",
		 "ערב_יום_הכפורים",	"ערב_פסח",
		 "ערב_סוכות"},
		{ /// begin hebrew short
		 "חול",
		 "א_ר\"ה",		 "ב'_ר\"ה",
	...
	case HDATE_STRING_OMER:
				if (index > 0 && index < 50)
				{
					if (index < 11)
					{
						if (index == 2)  return_string_len = asprintf(&return_string, "%s_", count_days[0]);
						else return_string_len = asprintf(&return_string, "%s_", count_days[index]);
					}
					else
					{
						if ((index%10) == 0)
							return_string_len = asprintf(&return_string, "%s_", count_days[(index/10)+10]);
						else if (index < 20)
							return_string_len = asprintf(&return_string, "%s_%s_",
													count_days[(index%10)], count_days[10]);
						else return_string_len = asprintf(&return_string, "%s_%s%s_",
